...
void main() {
  ...
  vec3 col = rChannel * uColor1 + gChannel * uColor2 + bChannel * uColor3;
  float alpha = clamp(pattern + ridge * 0.4, 0.0, 1.0) * uBrightness;
...
export default function LineWaves({
  ...
  // Keep the props ref updated synchronously on every render
  propsRef.current = {
    speed,
    innerLineCount,
    outerLineCount,
  ...
  useEffect(() => {
    ...
    function update(time: number) {
      ...
      if (program) {
        ...
        // Apply dynamic props on every frame
        const p = propsRef.current;
        program.uniforms.uSpeed.value = p.speed;
        program.uniforms.uInnerLines.value = p.innerLineCount;
        program.uniforms.uOuterLines.value = p.outerLineCount;
        program.uniforms.uWarpIntensity.value = p.warpIntensity;
        program.uniforms.uRotation.value = (p.rotation * Math.PI) / 180;
        program.uniforms.uEdgeFadeWidth.value = p.edgeFadeWidth;
        program.uniforms.uColorCycleSpeed.value = p.colorCycleSpeed;
        ...
        program.uniforms.uEnableMouse.value = p.enableMouseInteraction;
